...
class SheetInterpreter:
    ...
    def Interpreter(self) :
        ...
        self._output.append("package deploy;\n")
...
class DataParser:
    ...
    def _WriteData2File(self, data) :
        file_name = FILE_PREFIX + self._sheet_name.lower() + ".bytes"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()

    def _WriteReadableData2File(self, data) :
        file_name = FILE_PREFIX + self._sheet_name.lower() + ".txt"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()
...
if __name__ == '__main__' :
    ...
    if len(sys.argv) > 3 :
        FIELD_NAME_ROW = int(sys.argv[3]) - 1

    if len(sys.argv) > 4 :
        FIELD_COMMENT_ROW = int(sys.argv[4]) - 1
        FIELD_DATA_ROW = FIELD_COMMENT_ROW + 1
